...
class ImagenetClassificationCallback:
    ...
    def tokenize(self, tokenizer, examples, device):
        ...
        input_ids = [
            [tokenizer.cls_token_id] + ids for ids in encoding_inputs["input_ids"]
        ]
    ...
    def zeroshot_classifier(self, model, tokenizer, classnames, templates):
        ...
        for classname in tqdm(classnames):
            ...
            class_embeddings = (
                model.get_text_features(**self.tokenize(tokenizer, texts, model.device))
                .detach()
                .cpu()
                .numpy()
            )
